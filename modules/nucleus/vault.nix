# modules/nucleus/vault.nix — the secrets broker's own uid + system service.
#
# Workstream VAULT, P-V4 (deployment; ~/.claude/plans/functional-singing-boole.md
# "Workstream VAULT" section). Provisions the target topology the vault
# design settled on: the broker runs as ITS OWN system user `aoide-vault`
# (never the operator's uid, never root), vault home `/var/lib/aoide-vault`
# is `0700` vault-uid, and the socket `/run/aoide-vault/vault.sock` is the
# ONLY door — `0660`, group `aoide-vault-access`, whose membership is this
# option's `aoide.vault.members` list.
#
...
# a nix-dependent unit here is fine even though the `aoide vault serve`
# BINARY itself stays nix-independent (plain Rust + unix socket + shell-outs,
# `pkgs/aoide/crates/vault/src/home.rs`'s module doc — this file is what
...
# in `pkgs/aoide/crates/vault/README.md`'s "Deployment" section instead —
# this module is one packaging of that same binary, not a requirement of it.
#
# Off by default, gated on `aoide.vault.enable` (nucleus itself carries no
...
# holds regardless of deployment (a scratch `AOIDE_VAULT_HOME` run by hand
# gets the same `0660`). Making that mode meaningful — giving the socket the
# RIGHT gid — is this module's job: `Group=aoide-vault-access` below is the
...
# a vault has no business caring whether a desktop session exists), and
# `aoide vault serve` BLOCKS forever on its accept loop (`broker::serve`'s
...
lib.mkIf (config.aoide.enable && config.aoide.vault.enable) {

  # ── The broker's own uid + the two groups it needs ───────────────────────
  # `aoide-vault` (the service's own group, home-dir ownership) is separate
  # from `aoide-vault-access` (the socket's group — who may CONNECT, wired
  ...
  # to `aoide.vault.members` also handed them the broker's OWN primary
  # group, which is one bit more privilege than "may reach the socket."
  users.groups.aoide-vault = { };
  users.groups.aoide-vault-access.members = config.aoide.vault.members;

  users.users.aoide-vault = {
    isSystemUser = true;
    group = "aoide-vault";
    description = "Aoide secrets broker (vault) — owns the vault home, never logs in";
    home = "/var/lib/aoide-vault";
  ...
  systemd.services.aoide-vault-serve = {
    description = "Aoide secrets broker (vault) — unix-socket JSON-lines daemon, own uid";
    ...
    serviceConfig = {
      ...
      # `aoide vault serve` blocks forever on its accept loop, so this is
      ...
      ExecStart = "${pkgs.aoide}/bin/aoide vault serve";

      # `/var/lib/aoide-vault`, created + owned by User/Group above, mode
      # 0700 — the broker's own `home::secure_dir` re-asserts 0700 on every
      # `serve` startup regardless (crates/vault/src/home.rs's module doc),
      # so this and the code agree rather than one depending on the other.
      StateDirectory = "aoide-vault";
      StateDirectoryMode = "0700";
      # `/run/aoide-vault`, mode 0750: owner (aoide-vault) rwx, group
      # (aoide-vault-access) r-x — group members need the `x` bit to reach
      # into the directory and connect to `vault.sock` inside it; the
      ...
      # The env tier exists exactly for this (crates/vault/src/home.rs's
      # module doc: "Set AOIDE_VAULT_SOCKET/AOIDE_VAULT_HOME ... on any host
      ...
      # StateDirectory/RuntimeDirectory above, so the vault home and socket
      ...
      # crates/vault/README.md's "Backend presets") reach their own stores
  ...
  # the vault home, so they must run AS the vault user — this module
  ...
  #   sudo -u aoide-vault aoide vault add <name> --backend <b> --key <k>
  #   sudo -u aoide-vault aoide vault grant <name> <consumer>
  #
  # AOIDE_VAULT_HOME/AOIDE_VAULT_SOCKET are not needed on these invocations
  # when they match this module's paths — `sudo -u aoide-vault` alone is not
  # enough to pick up `home::vault_home`'s env override (sudo does not carry
  # the CALLER's env by default), so the CODE default
  # (`/var/lib/aoide-vault`, `home.rs`) already matching this unit's explicit
