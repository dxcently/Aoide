...
- **Run**: Linux tests on `x86_64-unknown-linux-gnu`, plus `aoide-protocol`,
  ...
  `cargo test --all-targets` on those four is 188 (protocol) + 446 (storage)
  + 361 (secrets lib) + 2 (secrets e2e) + 32 (upkeep) passed / 0 failed /
  0 ignored. Two facts were asked of a NON-admin token as well, because the
...
- **Not run**: macOS/FreeBSD runtime checks, and the rest of the core's
  ...
  reports 47 errors: `aoide-client` 45 (`tunnel.rs` 22, `commands.rs` 14,
  `mcp_client.rs` 4, `daemon.rs` 3, `pair_watch.rs` 2), `aoide-storage` 1 (a
  client-side reference to `attest::connect_bounded`), and `aoide-conduct`,
  `aoide-server` and `aoide-cli` **none** — those three compile cleanly on
  that host today. The kinds are the Unix-only vocabulary and nothing else:
