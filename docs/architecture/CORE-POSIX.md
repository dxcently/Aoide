...
- **Run**: Linux tests on `x86_64-unknown-linux-gnu`, plus `aoide-protocol`
  ...
  is 171 + 442 passed / 0 failed / 0 ignored. Two of those facts were asked
  of a NON-admin token as well, because both earlier runs used an elevated
  one: under `runas /trustlevel:0x20000` — `whoami /groups` reporting
  `BUILTIN\Administrators` as "Group used for deny only" — the
  directory-tighten test passes, so owner pinning through `SetSecurityInfo`
  needs no privilege on a standard-user token either. Reported, not measured:
  ...
  the same volume still works, and nothing here has run on such a volume. That covers the feed's 17
