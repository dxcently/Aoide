...
| `pair` | 2 | `ask` (inbound: collects a typed SAS code read off the requester's screen) and `confirm` (outbound: shows this instance's own derived SAS, a single Approve/Dismiss) — the pairing-ceremony popups behind `peer pair watch --popup`, sharing `secrets ask`'s six-box QML component — [[Pairing-Ceremony]] |
