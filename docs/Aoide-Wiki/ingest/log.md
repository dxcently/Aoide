...
- **Baton 3D DAG selection keybind** — `j/k` become camera pitch, so node selection moves to `n/p`. Recorded in [[songbook/Baton-3D-DAG]] as the design; the keys are unconfirmed against the rest of the baton bindings.
- **A dark `moonlight-sonata` counterpart to `sonata`** (khoa) — the light key's dusk sibling. Not built; cut from [[songbook/Pantheon-Grammar]] prose by the sweep and parked here.
