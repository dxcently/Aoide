...
- [[aoide-cli]] — answers: what the `aoide` binary exposes (the 37-command tree incl. `cover set`, the `graph` group, `conduct`, and `conductor`, which commands are real vs structured not-implemented stubs), how `schema --json` is the single source of truth the MCP tool list generates from, the exit-code conventions (0/1/2/64), how `graph focus` verifies window liveness and its five failure reasons, how it locates `drachma`, and the second `aoided` binary
...
- [[Quickshell]] — answers: what shell surfaces the QML runtime provides (nine registered, eight with a live QML body — session-graph is declared but has none today), why it replaces swaync/rofi/hyprlock/swww, how drachma hot-reloads at rehearsal, what the gadget-dock panel (`AoidePanel.qml`) holds, where the deployed QML tree lands (~/Aoide/qml, an open deploy-target question), and what the NotificationServer spike covers
