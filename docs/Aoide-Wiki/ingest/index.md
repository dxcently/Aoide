...
- [[Session-Graph]] — answers: how the project/session DAG works (nodes, anchors + spawned edges, longest-cwd-prefix anchoring, live-state merge), what the `aoide graph` commands do (view/emit/project/link/liveness-checked focus/prune), what the projects.json/graph.json v0 contracts hold, and how the DAG renders today (`graph view`/`--json` and the `aoide baton` TUI — the desktop's standalone DAG overlay and shared GraphModel are retired; the dock's Conductor gadget gives the agent-tree view instead)
...
- [[aoide-cli]] — answers: what the `aoide` binary exposes (the 37-command tree incl. `cover set`, the `graph` group, `conduct`, and `baton`, which commands are real vs structured not-implemented stubs), how `schema --json` is the single source of truth the MCP tool list generates from, the exit-code conventions (0/1/2/64), how `graph focus` verifies window liveness and its five failure reasons, how it locates `drachma`, and the second `aoided` binary
