...
- [[drachma]] — answers: what the Node drachma mint does (lint/resolve/emit subcommands, the three emitters, the atomic stage write), how it wraps Style Dictionary for the tiered resolver + null→palette fallback, and where it lives in nix (`packages.drachma`, the overlay attr)
...
- [[design/Ricing-Protocol|Ricing Protocol]] — answers: the creation/application split (deriving a base16 vs Stylix fanning it out), the mandatory light/dark vision-check, and where per-song design memory belongs (the songbook), worked through the `sonata` light key
- [[design/Pantheon-Grammar|Pantheon Grammar]] — answers: the first-song visual language (hollow wireframe depth recipe + `GadgetFrame` constants, the meaning-bearing glyph grammar, the multicolor role seam), plus the round-by-round design log through the `sonata` light rework
- [[design/Conductor-Channel|Conductor Channel]] — answers: how a wrapped agent session is commanded (PTY control socket, the gated `graph send` injection door, parent-autogate)
- [[design/Baton-3D-DAG|Baton 3D DAG]] — answers: the PLANNED ratatui 3D-wireframe DAG view for `aoide baton` (not yet implemented — a build plan)
