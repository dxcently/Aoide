...
The rebuild is the most carefully gated step in the [[Self-Ricing]] loop (`adopt` → commit → rebuild). NixOS makes every `switch` recoverable — a new generation is written and the old one stays bootable — so the gate governs authority and intent, not damage prevention.
...
Until `aoide.rebuild` is enabled, rebuilds follow the house rule (*you propose; the user admits; git records* — `AGENTS.md`):
...
This is the only mode until the capability is deliberately turned on.
...
What actually contains a compromised agent is how narrow the granted capability is — one pinned unit, nothing else — combined with NixOS rollback and the audit log. Security comes from scope + recoverability + audit, not from a typed secret.

## Rollback

Nothing here defeats recovery. Every `switch` is a new generation; the previous one stays in the boot menu and via `nixos-rebuild --rollback`. A bad `test` reverts on reboot. On yomi-strix the pre-Aoide [[dxflake]] generation is retained in systemd-boot as the rollback (see [[Codebase]]). The gate governs *who may act and whether they should*; the generation history guarantees any act can be undone.
