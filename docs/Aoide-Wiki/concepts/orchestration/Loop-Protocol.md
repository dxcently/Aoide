...
guidance: a long-running executor binds to a `graph spawn` session —
...
force `graph spawn` regardless of how short the work looks:

- the role crosses a harness or model boundary the orchestrator itself isn't
  running on,
- the brief needs mid-flight steering,
- the user needs to observe the role directly — a `graph spawn` session
