...
- [[aoide-cli]] — the `aoide` CLI trunk (69 commands per `aoide schema --json`) and the `aoided` daemon binary; `schema --json` is the single source of truth behind the MCP façade. Paint commands (`rice`, `cover`, `livery`, `screen`, …) are `lyra`'s
