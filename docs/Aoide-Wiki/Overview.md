...
- [[aoide-cli]] — the `aoide` binary: the CLI trunk (54-command tree incl. `cover set`, `rice compose`, the `rice mode` staging/declarative toggle, the `livery` group, the `graph` group, `conduct`, and `conductor`), `schema --json` as single source of truth, the stdio MCP façade, structured exit codes, and the `aoided` daemon binary
