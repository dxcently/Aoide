...
Two instances become peers through a mutual, two-code ceremony, named here by seat: the requester runs `aoide pair <target>`, raising a request; the approver runs `aoide pair <id>`, typing the code read off the requester's screen, and its own peer record commits — its screen then shows a second, different reply code; the requester's still-running `aoide pair` types that reply code, and its own record commits last. Both legs are typed, never a bare yes/no, gated identically whether driven from a plain terminal or `aoide pair watch --popup`'s Quickshell dialogs (`lyra pair ask` to type, `lyra pair show` to display). What it buys: a verified peer record carrying an `allows` set (`read`, `spawn`), a cross-host `peer list` roster with `peer pull`/`peer spawn` reaching into it, and — for a peer whose own door binds loopback-only — a reach-back over an ssh `via` hop, never a routable bind. → [Pairing Ceremony](docs/Aoide-Wiki/concepts/orchestration/Pairing-Ceremony.md), [Doors & Peers](docs/Aoide-Wiki/concepts/cli/Doors-and-Peers.md).
