...
> Status — walking skeleton. The desktop, the flake, the `graph` group, the daemon, and the theming fan-out are **real and running live** (yomi-strix is switched onto this flake). A handful of verbs (`rice gen`/`adopt`/`transpose`, the `content` group, `make`, `update`, `onboard`) are **structured stubs** that return exit `64` (`not-implemented`) with the right shape — the trunk is wired, the muscle is being grown. See [`aoide-cli`](docs/Aoide-Wiki/entities/aoide-cli.md) for exactly which.
