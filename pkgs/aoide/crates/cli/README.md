...
- `registry` — the golden command-path snapshot test (69 paths).
...
69 command paths (core's headless-capable, agent-orchestration surface: the
project/session graph, A2A, peers, presence, the daemon, its own event bus
(`events tail`), usage, hooks, the message inbox, the secrets broker). Never depends on
