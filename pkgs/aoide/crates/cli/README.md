...
- `registry` — the golden command-path snapshot test (70 paths).
...
70 command paths (core's headless-capable, agent-orchestration surface: the
project/session graph, A2A, peers (including the `peer hub` designation,
P-D5), presence, the daemon, its own event bus (`events tail`), usage,
hooks, the message inbox, the secrets broker). Never depends on
