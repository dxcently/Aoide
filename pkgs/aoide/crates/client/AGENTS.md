...
- **`mail_export` is READ-ONLY on the mailbase and clamps everything that
  ...
  carrying a newline ends the block. **Blocks are SENDS, not copies, and this
  rule is load-bearing too:** a `--to`/`--cc` send files one copy per mailbox and
  each copy is sealed on its own (its own `sig`, `msgid`, `minted_at`,
