...
- `peer_store` — the peer-federation registry + pull cache (CONTRACTS.md §7).
  ...
  nickname shape `valid_peer_name` requires, for `peer pair`'s url arm's
...
- `pairing` — the pairing ceremony's own park-and-approve state (P-P2,
  ...
  written by `peer pair`), keyed by STABLE 8-hex-char ids (never
  ...
  is uncapped (operator-created, one per `peer pair` call). Every
  ...
  explicit `--via`, or `peer pair`'s hostname arm/bare `pair`'s src_addr-derived
  default) forward to
