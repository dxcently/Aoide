//! This crate's own CLI commands — `aoide usage`, `aoide identity`, and
//! `aoide config`:
//!
...
//!
//! - `aoide identity` and `aoide config` — thin handlers over
//!   [`crate::identity`]/[`crate::config`]; each store's own module doc
//!   carries its shape and invariants, this file only parses invocations
//!   and renders output.
//!
//! `aoide mail`'s CLI surface used to live here too (messaging plan P-M1)
//! but moved to `aoide_client::commands::register_mail` at P-M2: mail now
//! dials another node to deliver (the outbox drain), and this crate sits
//! below `aoide-client` in the DAG and cannot depend on it. The store
//! itself — envelope/entry types, the canonical header, signing, the
//! lock, the migration, the origin-signature check, deposit — stays here
//! in [`crate::mail`], unmoved; only the command handlers crossed.
