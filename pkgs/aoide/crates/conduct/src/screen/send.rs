...
//!     [`crate::graph::session_send`] DIRECTLY (a same-process function call
...
//!
//! ── The cross-crate seam (khoa's brief, 2026-08-16) ─────────────────────
//! `aoide-conduct` (this crate) did not previously depend on `aoide-client`
...
//! the session-graph/capture domain (conduct) and the outbound A2A driver
//! (client) in the SAME function, and the alternative — reimplementing the
//! curl/JSON-RPC send here, or moving `screen send`'s registration up to the
//! `cli` crate out of step with every other `screen *` verb — would either
//! fork a second ungated injection path or break the established "all
//! `screen` verbs register together in `conduct::commands::screen`" pattern.
//! So `conduct`'s `Cargo.toml` now depends on `aoide-client` (one new edge;
//! `client` has no dependency back on `conduct`, so this does not cycle) and
//! `handle_agent_send` was bumped from crate-private to `pub` — a
//! visibility-only change, its body untouched. Bends the charter's
//! stated independence a little; flagged here rather than papered over.
//!
...
/// Wrap an inner [`Outcome`] (from [`crate::graph::session_send`] or
...
/// [`crate::graph::session_send`] directly. `submit: true` is FIXED, not a
...
fn send_to_session(inv: &Invocation, ctx: &SendCtx, id: &str, message: &str, yes: bool, audit_log: Option<&str>) -> Outcome {
    ...
    let inner = crate::graph::session_send(&sub_inv);
...
mod tests {
    ...
    fn write_conductable_session(id: &str, socket: &Path) {
        let mut file = crate::graph::SessionsFile::default();
        file.sessions.push(crate::graph::SessionRecord {
            session_id: id.to_string(),
            conductable: Some(true),
            socket: Some(socket.to_string_lossy().into_owned()),
            ..Default::default()
        });
        crate::graph::write_stage(&crate::graph::sessions_path(), &file).unwrap();
    }
