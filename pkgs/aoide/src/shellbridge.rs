...
///
/// `seed_if_absent` is additionally re-exported (beyond the symbols named in
/// the Phase 3a plan) because `run()` below — the socket-loop code that
/// stays in root through Phase 3b — calls it directly; without this the
/// shim would leave `run()` unable to compile.
