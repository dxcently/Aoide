...
// Usage · Terminals · Meters · Power — are stacked vertically at their NATURAL
// sizes inside a scrolling body; the chrome around them is a closed codex seen
// edge-on: a marble board with the hard plum border + inset gold keyline, a
// carved spine down the inner edge, and a stack of page-edge striations (the
// fore-edge) down the OUTER, screen-facing edge. This REPLACES the fold-out
// journal presentation (preserved separately as AoideJournal.qml); no fold, no
...
// to every loaded widget unconditionally (the universal widget contract,
// slots.md); Conductor/Terminals additionally receive `shared` as a slot
// extra (`extraProps: ({ shared: root.shared })`), the one thing not every
// slot gets.
//
...
PanelWindow {
    ...
    // the song-slot resolver — every WidgetSlot anchor below needs it to
    // resolve its own slot
            ...
            Item {
                ...
                Flickable {
                    ...
                    Column {
                        ...
                        // wired with `shared` (which it declares) as a slot extra.
                        WidgetSlot {
                            slot: "conductor"
                        ...
                        // sonata's usage.qml self-collapses its own implicitHeight to
                        // zero with no data, so the anchor reads the loaded item's
                        // SIZE rather than reaching into the child for a `hasData`
                        // property — an invisible Column child collapses to zero
                        // footprint (no gap) rather than leaving a hole between
                        // Conductor and Terminals.
                        //
                        // REMOVED from the dock 2026-08-12 (the User): the claude
                        // stele is out of the column for now. The block below is the
                        // exact re-enable (the loaded widget itself is untouched, and
                        // implicitHeight is its own binding, so nothing dangles):
                        //
                        //   WidgetSlot {
                        //       slot: "usage"
                        //       width: root.gadgetW
                        ...
                        // state/usage.json exists — sonata's usage.qml collapses its
                        // own implicitHeight to zero with no data (Phase 2), so the
                        // anchor reads the loaded item's SIZE rather than reaching
                        // into the child for a `hasData` property.
                        WidgetSlot {
                            slot: "usage"
                            width: root.gadgetW
                            livery: root.livery
