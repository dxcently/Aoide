// power.qml — sonata's "power" slot: THE POWER, a CORINTHIAN temple in the
...
// score; the facet original stays in place until a later phase retires it,
// so this file and PowerVitalsGadget.qml are momentarily twins. No anchor
// embeds `WidgetSlot { slot: "power" }` in the shipped dock yet — that lands
// with the real dock host, not here (see slots.md for the anchored
// catalog's own discipline: a slot is only listed there once a real anchor
// is wired).
//
import QtQuick
import Quickshell
...
//                  (the User, 2026-08-17: the old rest-notation 𝄽…𝅂/𝆑/𝄮 icons
...
Item {
    ...
    required property var bridge           // ShellBridge — WidgetSlot._songProps() always
                                            // passes this; unused today, declared per the
