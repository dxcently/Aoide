// usage.qml — sonata's "usage" slot: THE LEDGER, a claude.ai usage stele in
...
Item {
    ...
    // degraded (live-stub) state is a short panel, not an empty box. Through a
    // slot this ALSO carries the old host-side `visible: hasData` inversion —
    // zero while there's no data, so the anchor's own implicitHeight-driven
    // visibility (Phase 3) collapses this to nothing rather than an empty box.
    implicitHeight: hasData ? (body.implicitHeight + 26) : 0
    ...
    function requestRefresh() {
        ...
        // (which never scales) — the User: the two spinners must look identical mid-spin.
        ...
        Column {
            ...
            Item {
                ...
                Text {                                // the Claude spark — a rayed
                    ...
                    // (the User: "should play 1 whole loop first").
                    ...
                    // activeSpin (the User: match Conductor's hookTag exactly —
            ...
            Canvas {
                ...
                onPaint: {
                    ...
                    // line below (the User's "overlapping rows"). An astragal's
            ...
            // liveSection's old slot in the outer body Column (the User:
